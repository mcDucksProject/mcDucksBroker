...
class StrategyKlingerHybrid(IStrategy):
    INTERFACE_VERSION = 2
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_buy = self.do_indicators(self.dp.get_pair_dataframe(pair=metadata['pair'], timeframe=self.informative_timeframe_buy), metadata)
        [informative_buy["bkvo"],informative_buy["bks"]] = [informative_buy["kvo"],informative_buy["ks"]]
        informative_sell = self.do_indicators(self.dp.get_pair_dataframe(pair=metadata['pair'], timeframe=self.informative_timeframe_sell), metadata)
        [informative_sell["skvo"],informative_sell["sks"]] = [informative_sell["kvo"],informative_sell["ks"]]

        dataframe = merge_informative_pair(dataframe, informative_buy, self.timeframe, self.informative_timeframe_buy, ffill=True)
        dataframe = merge_informative_pair(dataframe, informative_sell, self.timeframe, self.informative_timeframe_sell, ffill=True)
        skip_columns = [(s + "_" + self.informative_timeframe_buy) for s in ['date', 'open', 'high', 'low', 'close', 'volume']] + \
                       [(s + "_" + self.informative_timeframe_sell) for s in ['date', 'open', 'high', 'low', 'close', 'volume']]
        dataframe.rename(columns=lambda s: s.replace("_{}".format(self.informative_timeframe_buy), "") if (not s in skip_columns) else s, inplace=True)
        dataframe.rename(columns=lambda s: s.replace("_{}".format(self.informative_timeframe_sell), "") if (not s in skip_columns) else s, inplace=True)
